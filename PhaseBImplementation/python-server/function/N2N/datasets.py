...
from function.N2N.utils import load_hdr_as_tensor
...
def load_dataset(root_dir, redux, params, shuffled=False, single=False, noisy_source=False):
    ...
    dataset = NoisyDataset(root_dir, redux, params.crop_size,
        clean_targets=params.clean_targets, noise_dist=noise, seed=params.seed, noisy_source=noisy_source)
...
class NoisyDataset(AbstractDataset):
    ...
    def __init__(self, root_dir, redux, crop_size, clean_targets=False,
        noise_dist=('gaussian', 50.), seed=None, noisy_source=False):
    ...
    def _add_noise(self, img):
        ...
        elif self.noise_type == 'brownGaussian':
            mean = 0
    ...
    def _corrupt(self, img):
        ...
        if self.noise_type in ['gaussian', 'poisson', 'brownGaussian']:
    ...
    def __getitem__(self, index):
        ...
        # Corrupt source image if no_noise flag is not set
